...
int main()
{
    ...
    // При отладке: {left = -135812528, right = 32767, top = -9048, bottom = 32767}

    // Поля содержат неопределенные значения, так как не было явной инициализации.
    // Будет вызываться неявный конструктор по умолчанию и неявный деструктор. Оба определены компилятором автоматически.

    // В этом случае:
    //  Конструктор не инициализирует поля типа int (оставляет неопределенные значения).
    //  Деструктор ничего не делает (нет ресурсов для освобождения).

    // В общем случае:
    //  Неявный конструктор по умолчанию:
    //      Для членов-объектов классов вызывает их конструкторы по умолчанию.
    //      Для встроенных типов оставляет значения неопределенными.
    //      Для массивов применяет эти правила к каждому элементу.

    //  Неявный деструктор:
    //      Для членов-объектов классов вызывает их деструкторы (в обратном порядке объявления).
    //      Для встроенных типов ничего не делает.
    //      Генерируется компилятором всегда, если деструктор не объявлен явно.
    ...
    // После объявления конструктора с параметрами компилятор перестаёт автоматически генерировать конструктор по умолчанию.
...

    {
        ...
        Rect rect1(1, 2, 3, 4); // Конструктор с параметрами (определен вручную)
        Rect rect2 = rect1; // Неявный конструктор копирования (определен компилятором)
        Rect rect3(rect1); // Неявный конструктор копирования (определен компилятором)
        
        // В этом случае конструктор копирования создает побитовую копию всех полей.
        // В общем случае конструктор копирования создает побитовую копию всех полей встроенных типов,
        //  а для объектов-членов вызывает их конструкторы копирования.
    } // Деструкторы вызываются 3 раза здесь в порядке обратном созданию объектов
    ...
    // Деструктор - специальный метод класса, применяющийся для освобождения памяти, занимаемой объектом.
    // Вызывается автоматически, когда объект выходит из области видимости.
    // В классе Rect компилятор неявно генерирует деструктор.

    // В этом случае ничего не делает.
    // В общем случае для объектов-членов вызывает их деструкторы, ничего не делает для встроенных типов.
    // Генерируется компилятором всегда, если не объявлен явно.
    ...
    {
        std::cout << "Задание 1.6."<< std::endl;
        Rect r1; // [1] Конструктор по умолчанию
        Rect *pR = new Rect(1,2,1,2); // [2] Конструктор с параметрами
        {
            Rect r2(r1); // [3] Конструктор копирования
            Rect arRect[2]; // [4,5] Конструктор по умолчанию (2 раза)
            for(int i = 0; i < 3; i++)
            {
                static Rect r3 (i,i,i,i) ; // [6] Конструктор с параметрами (1 раз)
                Rect r4(*pR); // [7,9,11] Конструктор копирования (3 раза)
                Rect r5(i,i,i,i); // [8,10,12] Конструктор с параметрами (3 раза)
                // Деструкторы [8] и [7], [10] и [9], [12] и [11]  (в конце каждой из трех итераций)
            }
        }  // Деструкторы [5], [4], [3] (при выходе из внутреннего блока)
        delete pR; // Деструктор [2]
    } // Деструктор [1] (при выходе из внешнего блока)
    // Деструктор [6] (при завершении работы программы)

    // Общее количество вызовов именно в этом блоке
    //  Конструторы: 12. Деструкторы: 11. (Деструктор [6] вызывается не в этом блоке кода)
    /**
    ...
    // ‘int Rect::left’ is private within this context
    // Все поля объявлены как private, что означает доступ к ним только из методов самого класса Rect.
    // Функция main() не является членом класса, поэтому прямой доступ к приватным полям запрещен.
    // Нужно либо сделать поля public, либо добавить публичные методы для доступа к объектам класса.
    ...
    /*
    * Функция-член вызывается в контексте объекта класса и имеет доступ ко всем
    * его полям (включая приватные) через неявный указатель this. Обычная функция
    * существует независимо и может обращаться только к публичному интерфейсу класса.
    *
    * Функция-член объявляется внутри класса, может иметь модификаторы доступа
    * (public/private/protected) и спецификаторы (const, virtual, static). Она
    * определяется с квалификацией имени класса. Обычная функция существует в
    * глобальной области видимости или пространстве имён.
    *
    * Только функция-член может быть виртуальной, что позволяет реализовать
    * полиморфизм. Она является частью инкапсуляции класса, тогда как обычная
    * функция работает с классом извне через его публичный интерфейс.
    */
    ...
    {
        std::cout << "Задание 1.9."<< std::endl;
        Rect r(10, 20, 10, 20);
        r.inflate(5);
        std::cout << r.get_left() << std::endl;
        r.inflate(2, 3);
        std::cout << r.get_left() << std::endl;
        r.inflate(1, 2, 3, 4);
        std::cout << r.get_left() << std::endl;
    ...
    {
        std::cout << "Задание 1.10."<< std::endl;
        Rect r1(1, 5, 1, 5), r2(3, 8, 3, 8);
    ...
    {
        std::cout << "Задание 1.11."<< std::endl;
        Rect r(3, 8, 52, 42);
        
        std::cout << "Ширина: " << r.get_width() << std::endl;     
        std::cout << "Высота: " << r.get_height() << std::endl;   
        std::cout << "Площадь: " << r.get_square() << std::endl;  
        
        r.set_width(50);
        r.set_height(40);
        
        std::cout << "Новая ширина: " << r.get_width() << std::endl;
        std::cout << "Новая высота: " << r.get_height() << std::endl;
        std::cout << "Новая площадь: " << r.get_square() << std::endl;
        
        std::cout << "Проверка на отрицательные размеры:"<< std::endl;
        r.set_width(-10);
        r.set_height(-5);
        std::cout << "Новая ширина: " << r.get_width() << std::endl;
        std::cout << "Новая высота: " << r.get_height() << std::endl;
...

    {
        ...
        // По условию: 2 бочки известного объема (предположим 10 литров)
        // Начальное количество жидкости выбираем равным объему для простоты
        // Первая: полная спирта 96%
        // Вторая: полная воды 0%
        Barrel alch(10.0, 10.0, 96.0); // бочка со спиртом
        Barrel water(10.0, 9.5, 0.0); // бочка с водой

        int iteration = 0;
        double ladle = 1.0;  // кружка 1 литр

        // Пока концентрация спирта в бочке alch > 50%
        while(alch.get_alcohol_percent() > 50.0)
        {
            iteration++;
            
            // Из бочки со спиртом в бочку с водой
            alch.pour_to(water, ladle);
            
            // Из бочки с водой в бочку со спиртом
            water.pour_to(alch, ladle);
        
            std::cout << "Итерация " << iteration << ": " << alch.get_alcohol_percent() << "%" << std::endl;
        }

        std::cout << "\nРезультат:" << std::endl;
        std::cout << "Концентрация спирта в первой бочке стала < 50% на итерации: " 
                << iteration << std::endl;
    }
...

    {
        ...
        try {
            // Создаем матрицу преобразования 2x2
            Matrix fib_transform(2);
            fib_transform.set(0, 1, 1);
            fib_transform.set(1, 0, 1);
            fib_transform.set(1, 1, 0); // ?
            
            std::cout << "Матрица преобразования:" << std::endl;
            fib_transform.print();
            
            // Начальный вектор [F(1); F(0)] = [1; 0]
            Matrix fib_vector(2, 1, 0.0);
            fib_vector.set(0, 0, 1.0);  // F(1) = 1
            
            std::cout << "\nНачальный вектор:" << std::endl;
            fib_vector.print();
            
            // Вычисляем первые 40 чисел Фибоначчи
            std::cout << "\nЧисла Фибоначчи:" << std::endl;
            for (int i = 1; i <= 40; ++i) {
                // Текущее значение F(i) находится в первой строке вектора
                std::cout << "F(" << i << ") = " << fib_vector.get(0, 0) << std::endl;
                
                // Умножаем матрицу преобразования на вектор
                Matrix new_vector = fib_transform.multiply(fib_vector);
                fib_vector = new_vector;  // Используем оператор присваивания ?
            }
            

            std::cout << "\nДемонстрация других операций с матрицами:" << std::endl;
            ...
            // Сложение
            A.add_in_place(B);
            std::cout << "A += B (результат):" << std::endl;
            A.print();
            
            // Отрицание
            A.negate();
            std::cout << "Отрицание A:" << std::endl;
            A.print();
    ...
    {
        std::cout << "\nЗадание 2.3." << std::endl;
        
        // Демонстрация конструкторов и методов
        MyString s1;
        std::cout << "s1 (пустая): ";
        s1.print();
        std::cout << std::endl;
        ...
        std::cout << "s2[0] = " << s2.get(0) << std::endl;
        
        s2.set(0, 'h');
        std::cout << "s2 после set: ";
        s2.print();
        std::cout << std::endl;
        
        s2.set_new_string("New string");
        std::cout << "s2 после set_new_string: ";
        s2.print();
        std::cout << std::endl;
...

    {
        ...
        std::cout << "Введите текст для переноса:" << std::endl;
        input.read_line();
    ...
    /* Правки Makefile после изменений:

        build/debug.out: src/lab1.cpp src/rect.cpp src/barrel.cpp src/matrix.cpp src/my_string.cpp src/text_wrapper.cpp
            g++ -g -o build/debug.out src/lab1.cpp src/rect.cpp src/barrel.cpp src/matrix.cpp src/my_string.cpp src/text_wrapper.cpp
    ...
    */
    ...
    /**
     * Выполнен рефакторинг класса Rect:
     * - делегирование в перегруженных методах inflate()
     * - добавлены нормализованные геттеры для упрощения работы с координатами
     * - функция bounding_rect упрощена с использованием std::min/max
     * - исправлены get_width/get_height для возврата положительных значений
     * - изменены параметры bounding_rect на const& для избежания копирования
     */
     ...
     * - Все private-поля (было left/right/top/bottom, стало x/y/width/height)
     * - Все конструкторы (теперь конвертируют границы в x/y/width/height)
     * - Все методы доступа (get_left/right/top/bottom теперь вычисляются из x/y/width/height)
     * - Методы операций (inflate, move) под новое хранение
     * - Ожидаемые значения в test_rect_basic_methods: 
     *      assert(r1.get_right() == 1); // было 0, стало 1
     *      assert(r1.get_top() == 1); // было 0, стало 1
     * 
     * Что не пришлось менять:
     * - Публичный интерфейс (названия методов остались теми же)
     * - Функцию bounding_rect
     * - Функцию print_rect
     * - main() и тесты (кроме ожидаемых значений в тестах)
     */
    return 0;
}
